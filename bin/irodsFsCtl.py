...
def reset_cache(mount_path):
    ...
    if status != 0:
        print("failed to reset cache", file=sys.stderr)
    else:
        print("Done!")
    os.close(fd)
...
def show_connections(mount_path):
    ...
    if status != 0:
        print("failed to show connections", file=sys.stderr)
    else:
...
def ioctl(command, mount_path, oargs):
    if command is None:
        print("No command is given", file=sys.stderr)
    ...
    if command in COMMANDS:
        ...
        print("Unrecognized command: %s" % (command), file=sys.stderr)
